/*
=========================================================
Stored Procedure: Load Data to Silver Layer
=========================================================
Script Purpose: 
	This script creates a procedure to load data into the "silver" schema from 
	'bronze' Layer.
Do the following : 
	- Truncating the silver tables before loading data 
	- Use the "INSERT INTO" command to load the data from the 'bronze'  layer.
	
Parameters: 
	There are parameters in this Procedure
...
BEGIN 
	...
	BEGIN TRY 
		...
		SELECT 
			...
			CAST(
				LEAD(prd_start_dt)
					...
					AS DATE
			)AS prd_end_dt -- Calculate end dates as 1 day before next start date 
		FROM 
			bronze.crm_prd_info
		...
		SELECT 
			...
			CASE 
				...
				ELSE TRIM(cntry) -- Normalize and handle missing country codes 
			END AS cntry
		FROM 
			bronze.erp_loc_a101
	...
	BEGIN CATCH 
		...
		PRINT '================================================'
		PRINT 'Failed to Load Bronze Layer Data'
